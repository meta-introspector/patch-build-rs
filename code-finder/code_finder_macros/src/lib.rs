...
impl Parse for ModelShellScriptArgs {
    fn parse(input: ParseStream) -> SynResult<Self> {
        let content_key: Ident = input.parse()?;
        if content_key != "content" {
            return Err(input.error("Expected 'content' as the argument"));
        }
        let _colon1 = input.parse()?;
        let content = input.parse()?;
        Ok(ModelShellScriptArgs { content_key, _colon1, content })
    }
}
...
pub fn model_shell_script(input: TokenStream) -> TokenStream {
    ...
    let script_content = args.content.value();
    let span = args.content.span();

    let script_name = "embedded_script"; // No file name, so use a generic name
    let script_name_ident = Ident::new(&script_name.replace('.', "_"), span);
    let description = format!("Modeled embedded shell script");

    let ast = match parse(&script_content) {
        Ok(ast) => ast,
        Err(e) => {
            return quote_spanned! {span =>
                compile_error!(format!("Failed to parse shell script content: {}", #e));
            }
            .into();
        }
    };

    // ... (rest of the macro logic, converting AST to symbolic operations)
    // This part remains mostly the same, but instead of using full_path, it will use script_name.
