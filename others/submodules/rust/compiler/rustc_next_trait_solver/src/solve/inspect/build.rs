...
impl<D: SolverDelegate<Interner = I>, I: Interner> ProofTreeBuilder<D> {
    ...
    pub fn unwrap(self) -> inspect::Probe<I> {
...
impl<D: SolverDelegate<Interner = I>, I: Interner> EvaluationStepBuilder<D> {
    pub fn new_noop() -> EvaluationStepBuilder<D> {
        EvaluationStepBuilder { state: None, _infcx: PhantomData }
    }
    ...
    pub fn take_and_enter_probe(&mut self) -> EvaluationStepBuilder<D> {
    ...
    pub fn finish_probe(mut self) -> EvaluationStepBuilder<D> {
        if let Some(this) = self.as_mut() {
