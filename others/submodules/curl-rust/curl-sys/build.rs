...
struct NixPaths {
    ...
    openssl_lib: String,    // Path to OpenSSL library
}
...
impl NixPaths {
    fn default_nix_paths() -> Self {
        ...
        NixPaths {
            ...
            openssl_lib: get_nix_path_from_env(
                "NIX_OPENSSL_LIB",
                hardcoded_provider.openssl_lib(),
            ),
        }
    }
}
...
fn main() {
	...
	.define("USE_OPENSSL", None) // Define USE_OPENSSL
    ...
    if cfg!(feature = "rustls") {
        ...
        if windows {
	    println!("cargo:warning=SSLBRANCH2");
	    ...
	    println!("cargo:warning=SSLBRANCH1v2");

            cfg.define("USE_OPENSSL", None)
                .file("curl/lib/vtls/openssl.c");

            println!("cargo:rustc-cfg=link_openssl");
            // Explicitly use nix_paths for OpenSSL include
            cfg.include(&nix_paths.openssl_include);

//	    cfg.flag("--verbose");
	    
            // Explicitly tell cc::Build to link against OpenSSL libraries
            if !nix_paths.openssl_lib.is_empty() {
		println!("cargo:warning=SSLBRANCH1v2b");
                cfg.flag(&format!("-L{}", "/nix/store/61i74yjkj9p1qphfl7018ja4sdwkipx0-openssl-3.6.0/lib"));
            }
            cfg.flag(&format!("-L{}", nix_paths.openssl_lib));
            cfg.flag("-lssl");
            cfg.flag("-lcrypto");
            ...
            // // Explicitly tell Cargo to link against OpenSSL libraries
            // if !nix_paths.openssl_lib.is_empty() {
	    println!("cargo:rustc-link-search=native=/nix/store/61i74yjkj9p1qphfl7018ja4sdwkipx0-openssl-3.6.0/lib");
