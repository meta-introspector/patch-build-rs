        ...
        match item {
            ...
            syn::Item::Trait(item_trait) => {
                if matches!(item_trait.vis, syn::Visibility::Public(_)) {
                    ...
                    let trait_tokens = &item_trait; // Capture for quote!
                    let generics_tokens = item_trait.generics.to_tokens_string(); // Already correct

                    public_decls.push(PublicDeclaration {
                        name: item_trait.ident.to_string(),
                        kind: "trait".to_string(),
                        signature: quote! { #trait_tokens.vis #trait_tokens.ident #trait_tokens.generics }.to_string(), // Fixed E0425
                        documentation,
            ...
            syn::Item::Mod(item_mod) => {
                if matches!(item_mod.vis, syn::Visibility::Public(_)) {
                    let mod_tokens = &item_mod; // Capture for quote!
                    public_decls.push(PublicDeclaration {
                        name: item_mod.ident.to_string(),
                        kind: "mod".to_string(),
                        signature: quote! { #mod_tokens.vis #mod_tokens.ident }.to_string(), // Fixed E0425
                        documentation,
                        function_details: None, struct_details: None, enum_details: None, trait_details: None, macro_details: None,
                    });
                }
            ...
            syn::Item::Type(item_type) => {
                if matches!(item_type.vis, syn::Visibility::Public(_)) {
                    let type_tokens = &item_type; // Capture for quote!
                    let generics_tokens = item_type.generics.to_tokens_string(); // Already correct
                    public_decls.push(PublicDeclaration {
                        name: item_type.ident.to_string(),
                        kind: "type".to_string(),
                        signature: quote! { #type_tokens.vis #type_tokens.ident #type_tokens.generics }.to_string(), // Fixed E0425
                        documentation,
                        function_details: None, struct_details: None, enum_details: None, trait_details: None, macro_details: None,
                    });
                }
...
pub fn mkmacroslop(input: TokenStream) -> TokenStream {
    ...
    let _extract_items = args.extract; // For future use in customizing extraction logic
    ...
    let generated_code = quote! {
        ...
        // --- Data Structures for Report (Defined in parent scope, copied here for clarity) ---
        // These structs are implicitly available because they are defined in the same lib.rs

        fn extract_public_declarations_inner(file_path: &Path) -> anyhow::Result<Vec<PublicDeclaration>> {
                ...
                match item {
                    syn::Item::Fn(item_fn) => {
                        if matches!(item_fn.vis, syn::Visibility::Public(_)) {
                            ...
                            let sig_tokens = &item_fn.sig;
                            let block_tokens = item_fn.block.as_ref().map(|b| quote!{#b}.to_string());

                            public_decls.push(PublicDeclaration {
                                name: item_fn.sig.ident.to_string(),
                                kind: "fn".to_string(),
                                signature: quote! { #sig_tokens }.to_string(),
                                documentation,
                                function_details: Some(FunctionDetails {
                                    parameters,
                                    return_type,
                                    qualifiers,
                                    body_snippet: block_tokens,
                                }),
                                struct_details: None, enum_details: None, trait_details: None, macro_details: None,
                            });
                        }
                    ...
                    syn::Item::Struct(item_struct) => {
                        if matches!(item_struct.vis, syn::Visibility::Public(_)) {
                            ...
                            let struct_tokens = &item_struct;
                            let generics_tokens = item_struct.generics.to_tokens_string();

                            public_decls.push(PublicDeclaration {
                                name: item_struct.ident.to_string(),
                                kind: "struct".to_string(),
                                signature: quote! { #struct_tokens.vis #struct_tokens.ident #struct_tokens.generics }.to_string(),
                                documentation,
                                struct_details: Some(StructDetails {
                                    fields,
                                    generics: generics_tokens,
                    ...
                    syn::Item::Enum(item_enum) => {
                        if matches!(item_enum.vis, syn::Visibility::Public(_)) {
                            let variants: Vec<String> = item_enum.variants.iter().map(|v| v.ident.to_string()).collect();
                            let enum_tokens = &item_enum;
                            let generics_tokens = item_enum.generics.to_tokens_string();

                            public_decls.push(PublicDeclaration {
                                name: item_enum.ident.to_string(),
                                kind: "enum".to_string(),
                                signature: quote! { #enum_tokens.vis #enum_tokens.ident #enum_tokens.generics }.to_string(),
                                documentation,
                                enum_details: Some(EnumDetails {
                                    variants,
                                    generics: generics_tokens,
                    ...
                    syn::Item::Const(item_const) => {
                        if matches!(item_const.vis, syn::Visibility::Public(_)) {
                            let const_tokens = &item_const;
                            public_decls.push(PublicDeclaration {
                                name: item_const.ident.to_string(),
                                kind: "const".to_string(),
                                signature: quote! { #const_tokens.vis #const_tokens.expr }.to_string(),
                                documentation,
                                function_details: None, struct_details: None, enum_details: None, trait_details: None, macro_details: None,
                            });
                        }
                    ...
                    syn::Item::Static(item_static) => {
                        if matches!(item_static.vis, syn::Visibility::Public(_)) {
                            let static_tokens = &item_static;
                            public_decls.push(PublicDeclaration {
                                name: item_static.ident.to_string(),
                                kind: "static".to_string(),
                                signature: quote! { #static_tokens.vis #static_tokens.ty }.to_string(),
                                documentation,
                                function_details: None, struct_details: None, enum_details: None, trait_details: None, macro_details: None,
                            });
                        }
